...
class HouzzOrderImport(models.TransientModel):
    ...
    def do_order_import(self):
        ...
        orders = response_decode.get('Orders')
        if orders != None:
            self.save_order(orders, self.houzz.id)
        else:
            raise UserError(_('NO Orders on Selected Date Ranges'))
    ...
    def auto_import_order(self):
        ...
        today = date.today()
        houzzs = self.env['houzz.config'].search([], limit=1)
        for houzz_config in houzzs:
            ...
            response = houzz.get_orders(status='New', from_date=today, to_date=yesterday, format='json')
            response_decode = json.loads(response.decode('utf-8'))
            orders = response_decode.get('Orders')
            if orders != None:
                self.save_order(orders, self.houzz.id)
            _logger.info("Auto Scheduler Executed for the Houzz Order...")
        return True
    ...
    def create_product(self, sku_id):
        ...
        # pro_temp_obj = product_temp_obj.create({
        ...
        for order in orders:
            ...
            if not check_order:
                ...
                for i in order.get('OrderItems'):
                    ...
                    if not product:
                        pro_obj = self.sudo().create_product(sku_id=default_code)
                        if pro_obj != None:
